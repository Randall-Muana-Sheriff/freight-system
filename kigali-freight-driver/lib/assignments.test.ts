...
describe('pay on an assignment card', () => {
    it('carries the driver net through as a number', () => {
        ...
        const card = toDriverAssignmentCard({ ...order, driver_net: '14650.00', price_is_estimate: false });
        expect(card.payAmount).toBe(14650);
        expect(card.payIsEstimate).toBe(false);
    });

    it('marks pay as an estimate when the job has not been placed yet', () => {
        const card = toDriverAssignmentCard({ ...order, driver_net: 9350, price_is_estimate: true });
        expect(card.payAmount).toBe(9350);
    ...
    it('reports no pay rather than zero when a job has not been priced', () => {
        ...
        expect(toDriverAssignmentCard(order).payAmount).toBeNull();
        expect(toDriverAssignmentCard({ ...order, driver_net: null }).payAmount).toBeNull();
    });
    ...
    it('treats a missing estimate flag as a firm price, not an estimate', () => {
        ...
        expect(toDriverAssignmentCard({ ...order, driver_net: 100 }).payIsEstimate).toBe(false);
    });
});
