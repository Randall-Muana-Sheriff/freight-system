...
export function getVehicleIcon(vehicleType, status = 'normal') {
  const cacheKey = `${vehicleType || 'default'}|${status}`;
  const cached = vehicleIconCache.get(cacheKey);
  if (cached) return cached;

  const { glyph, size } = VEHICLE_TYPE_GLYPHS[vehicleType] || DEFAULT_VEHICLE_GLYPH;
