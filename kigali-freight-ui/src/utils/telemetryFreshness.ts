...
export type Freshness = 'unknown' | 'offline' | 'stale' | 'live';

export function classifyFreshness(lastSeen: string | null | undefined, now: number): Freshness {
...
export function formatLastSeen(lastSeen: string | null | undefined, now: number): string {
    if (!lastSeen) return 'unknown';
