...
export default function FleetAssetList() {
  ...
  const now = useNow();
  const [savingDriver, setSavingDriver] = useState(null);
  const [feedback, setFeedback] = useState(null); // { driverName, ok, message }
  ...
  const handleSnapAndSave = async (driverName) => {
    ...
    try {
      ...
      setFeedback({ driverName, ok: false, message: err.message || 'Failed to save' });
    } finally {
        ...
        return (
          ...
          >
            ...
            <div className="flex flex-col items-end gap-0.5">
              <button
                onClick={() => handleSnapAndSave(asset.driverName)}
                disabled={isSaving}
