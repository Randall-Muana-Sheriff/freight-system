      ...
      <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-1">
        {violations.length === 0 ? (
          ...
          violations.map((violation, i) => (
            <div key={violation.id ?? i} className={`p-2 border rounded text-[11px] flex flex-col space-y-0.5 ${violation.type === 'SPEED_VIOLATION' ? 'border-hazard/30 bg-hazard/10' : 'border-rust/30 bg-rust/10'}`}>
              <div className="flex justify-between font-bold text-paper">
                ...
                <span className="font-mono text-[9px] text-steel">{violation.enteredAt ? new Date(violation.enteredAt).toLocaleTimeString() : ''}</span>
              </div>
