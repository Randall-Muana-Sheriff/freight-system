...
export default function IncidentRegistry() {
  ...
  return (
    <div className="bg-panel border border-line/10 p-3 rounded-md space-y-2">
      <h3 className="text-micro font-bold text-rust uppercase tracking-wider flex items-center gap-1.5">
      ...
      <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-1">
        {violations.length === 0 ? (
          <div className="text-data text-steel italic p-2 text-center bg-ink/40 rounded border border-line/10">
            No active security or speed breaches detected.
          </div>
        ) : (
          violations.map((violation, i) => (
            <div key={violation.id ?? i} className={`p-2 border rounded text-data flex flex-col space-y-0.5 ${violation.type === 'SPEED_VIOLATION' ? 'border-hazard/30 bg-hazard/10' : 'border-rust/30 bg-rust/10'}`}>
              <div className="flex justify-between font-bold text-paper">
                ...
                <span className="font-mono text-micro text-steel">{violation.enteredAt ? new Date(violation.enteredAt).toLocaleTimeString() : ''}</span>
              </div>
              <div className="text-steel font-mono text-micro">{violation.description}</div>
            </div>
          ))
        )}
      </div>
