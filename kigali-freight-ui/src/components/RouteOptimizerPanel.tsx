...
interface RouteOptimizerPanelProps {
    onRouteOptimized?: (routes: OptimizedRouteGroup[]) => void;
...
export default function RouteOptimizerPanel({ onRouteOptimized, stopTargetMode, setStopTargetMode, newStopCoords }: RouteOptimizerPanelProps) {
    ...
    const [optimizedSequence, setOptimizedSequence] = useState<OptimizedRouteNode[]>([]);
    ...
    useEffect(() => {
        async function fetchStops() {
            try {
                const data = await apiFetch('/api/stops', { token: jwtToken }) as DeliveryStop[];
                if (Array.isArray(data)) setStops(data);
            } catch (err) {
                console.error('Failed to load pending stops', err);
            }
        }
        if (jwtToken) void fetchStops();
    }, [jwtToken]);
    ...
    const handleRunOptimization = async () => {
        ...
        setError(null);
        try {
            const data = await optimizeMultiStopRoute(depot, vehicles, stops as unknown as Parameters<typeof optimizeMultiStopRoute>[2], 100, jwtToken) as OptimizationResult;
            setResult(data);
    ...
    const handleMoveStopSequence = (index: number, direction: 'up' | 'down') => {
    ...
    return (
        <div className="bg-panel border border-line/10 p-4 rounded-md text-paper space-y-3">
            ...
            <form onSubmit={(e) => void handleAddStopSubmit(e)} className="space-y-2 bg-ink/60 p-2.5 rounded border border-line/10">
                ...
                {stops.map((s) => (
                    <div key={s.id} className="bg-ink/40 p-1.5 rounded flex justify-between items-center border border-line/10 text-steel">
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <button
                                type="button"
                                onClick={() => void handleDeleteStop(s.id)}
