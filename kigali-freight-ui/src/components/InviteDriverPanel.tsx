                ...
                <form onSubmit={(e) => void handleInvite(e)} className="bg-ink/60 p-3.5 rounded border border-line/10 space-y-2.5">
                    <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2.5 items-end">
                        <label className="block">
                            <span className="block text-micro text-steel/70 uppercase tracking-wider mb-1">Phone number</span>
                            <input
                                type="tel"
                        ...
                        <label className="block">
                            <span className="block text-micro text-steel/70 uppercase tracking-wider mb-1">Full name</span>
                            <input
                                type="text"
                        ...
                        <label className="block">
                            <span className="block text-micro text-steel/70 uppercase tracking-wider mb-1">Vehicle (optional)</span>
                            <select
                                value={form.vehicleId}
                                onChange={(e) => setForm((f) => ({ ...f, vehicleId: e.target.value }))}
                                className="w-full bg-panel border border-line/15 rounded px-2 py-1.5 text-data text-paper focus:outline-none focus:border-route"
                            >
                    ...
                    {drivers.map((d) => (
                        <div key={d.id} className="bg-ink/60 p-2.5 rounded border border-line/10 flex justify-between items-center">
                            <div className="truncate max-w-[280px]">
                                ...
                                <div className="text-micro text-steel">{d.staffId || `ID: ${d.id}`} · {d.phoneNumber || d.username}</div>
                            </div>
                            {d.phoneNumber ? (
                                <button
                                    type="button"
                                    ...
                                    className="text-micro font-bold uppercase tracking-wide text-rust hover:text-paper disabled:opacity-50 transition-colors whitespace-nowrap"
                                >
                                    {resettingId === d.id ? '...' : 'Reset PIN'}
                                </button>
                            ) : (
                                <span className="text-micro text-steel/60 uppercase tracking-wide whitespace-nowrap">Legacy account</span>
                            )}
                        </div>
                    ))}
                </div>
