// src/components/LiveFleetStatusPanel.jsx — per-order ETA/distance-remaining
...
export default function LiveFleetStatusPanel() {
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            setReport(await fetchLiveFleetStatus(jwtToken));
        } catch (err) {
            setError(err.message || 'Failed to load live fleet status.');
        } finally {
            setLoading(false);
        }
    }, [jwtToken]);
    ...
    return (
        <div className="bg-panel border border-line/10 p-3 rounded-md text-paper space-y-2">
            <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={load} disabled={loading} className="text-steel hover:text-paper disabled:opacity-50">
