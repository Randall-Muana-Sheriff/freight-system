// src/components/AdminControlCenterPage.jsx — full-screen admin console,
...
const ORDER_STATUS_ORDER = ['PENDING', 'ASSIGNED', 'PICKED_UP', 'IN_TRANSIT', 'ARRIVED', 'DELIVERED', 'CANCELLED'];

function StatCard({ icon: Icon, label, value, accent = 'text-route', sub }) {
  return (
...
function StatisticsSection({ jwtToken }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setStats(await fetchAdminStats(jwtToken));
    } catch (err) {
  ...
  const totalOrders = stats ? Object.values(stats.ordersByStatus).reduce((sum, n) => sum + n, 0) : 0;
...
function FleetPerformanceSection() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setReport(await fetchFleetPerformanceReport(jwtToken));
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
