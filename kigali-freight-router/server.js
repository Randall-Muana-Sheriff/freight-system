...
// Initialised by `node --import ./instrument.js` before this file loads;
// this import only borrows the configured client.
...
// Before the handler below, not after: Sentry's middleware captures the
// error and calls next(), so the app's own handler still shapes the client
// response exactly as it did. Registering it the other way round means the
// response is sent first and Sentry never sees the error at all.
//
...
process.on('unhandledRejection', (reason) => {
  ...
  // Deliberately not alerted to Telegram: rejections can arrive in bursts
  // from one failing dependency, and twenty phone notifications for one
  // fault trains people to ignore the channel. Sentry groups them into a
  // single issue with a count, which is the right shape for this.
...
process.on('uncaughtException', async (error) => {
  ...
  await Promise.race([
    Promise.all([
      ...
      // Events are queued in memory and sent on a timer, so a process that
      // exits immediately takes the report of its own death with it. flush
      // is inside the same race as the alert, under the same timeout, for
      // the same reason: the crash-exit must not wait on a network call.
      Sentry.flush(2500),
    ]),
