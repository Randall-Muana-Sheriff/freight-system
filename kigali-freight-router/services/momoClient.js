...
import { resolveSettlementCurrency } from '../utils/money.js';
...
function resolveCurrency(orderCurrency) {
    // The decision lives in utils/money.js so it can be unit tested without a
    // fresh process per case — CURRENCY_OVERRIDE is read from the environment
    // at module load, which is why this was never covered. This wrapper only
    // turns its refusal back into the MomoError callers already handle.
    const resolved = resolveSettlementCurrency(orderCurrency, CURRENCY_OVERRIDE);
    if (!resolved.ok) {
        throw new MomoError(resolved.message, { code: resolved.code });
    }
